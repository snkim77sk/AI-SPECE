...
def test_exact_contract_link_merges_into_unique_final_award_execution(monkeypatch):
    ...
    monkeypatch.setattr(contract_projection, "resolve_unique_final_award_key", lambda notice: "R26BK00000001|000|1|0")
    monkeypatch.setattr(
        contract_projection,
        "upsert_award_result",
    ...
    assert award_seen["key"] == "R26BK00000001|000|1|0"
...
def test_contract_keeps_notice_link_but_does_not_guess_ambiguous_award_execution(monkeypatch):
    ...
    links = []
    monkeypatch.setattr(contract_projection, "resolve_notice_key", lambda row: "R26BK00000001|000")
    monkeypatch.setattr(contract_projection, "resolve_unique_final_award_key", lambda notice: "")
    monkeypatch.setattr(
        contract_projection,
        "upsert_award_result",
    ...
    assert result["award_summary_key"] == ""
